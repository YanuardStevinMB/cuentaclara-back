...
-- EXECUTE defers SQL parsing to runtime so column-existence check is honored
-- (PL/pgSQL compiles static SQL at block parse time, making IF guards useless for missing columns)
DO $$
BEGIN
    IF EXISTS (
        ...
        EXECUTE 'CREATE INDEX IF NOT EXISTS ix_categories_user ON categories (user_id, type, active)';
        EXECUTE $q$
...
BEGIN
    IF EXISTS (
        ...
        EXECUTE 'CREATE INDEX IF NOT EXISTS ix_movements_user_date ON movements (user_id, movement_date DESC) WHERE deleted_at IS NULL';
        EXECUTE 'CREATE INDEX IF NOT EXISTS ix_movements_user_type ON movements (user_id, type, movement_date) WHERE deleted_at IS NULL';
    END IF;
END $$;
